...
/*練習メニュー*/
typedef enum { Term, KeyPos, KeyPosComp, Clang, Conversation, Japanese_plus, Expected, Blocktype2, Blocktype3, Invalid } Menu;
...
/*--- Ｃ言語／英会話トレーニング ---*/
void word_training(const char* mes, const char* str[], int n,int select)
{
	int stage;
	int tno, mno;					/* 文字数・ミス回数 */
	int qno, pno;					/* 問題番号・前回の問題番号 */
	...
	save_data(select, (double)(end - start) / CLOCKS_PER_SEC, (double)tno * 60 / (double)(end - start) / CLOCKS_PER_SEC, mno);		/*セーブ用関数に情報を伝える順番は(　switch用変数　所要時間　1分間当たりのタイプ速度　　ミス回数)*/
}

void word_training_plus(const char* mes, const char* str[], int n, int select)
{
	int stage;
	int tno, mno;					/* 文字数・ミス回数 */
	int qno, pno;					/* 問題番号・前回の問題番号 */
...
int main(void)
{
	...
	do {
		...
		case Clang:						/* Ｃ言語の単語 */
			word_training("Ｃ言語の単語", cstr, cn,2);
			break;

		case Conversation:				/* 英会話 */
			word_training("英会話の文書", vstr, vn,3);
			break;
			case Japanese_plus:
				word_training_plus("英会話の文書の日本語付き", vstr, vn,4);
				break;
